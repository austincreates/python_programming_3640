...
class Text(object):
    ...
    def apply_move(self, move):
        ...
        '''
        new_string = ""
        ref = self.create_moved_dict(self.move)
        for letter in self.text:
            if letter in alpha:
                new_string += ref[letter]
            else:
                new_string += letter
        return new_string
...
class CipherText(Text):
    ...
    def decrypt_text(self):
        ...
        highest = 0
        best_move = 0
        for love in range(26):
            count = 0     
            self.move = love
            moved = self.apply_move(love)
            #print("The move for {} is {}".format(love,moved))
            words = moved.split()
            for word in words:
...
result = Text("Yo face is not my face")
print(result.create_moved_dict(3))
result.move = 9
print (result.apply_move(result))
